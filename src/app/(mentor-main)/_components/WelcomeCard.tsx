...
const SingleCard = () => {
  ...
  const card = {
    ...
    gradient: "from-purple-200 to-blue-400",
    textAccent: "text-blue-600",
    buttonGradient: "from-blue-300 to-blue-500",
    image: "/element7.png",
  };

  return (
    <div className="relative w-full max-w-[900px] mx-auto p-3">
      <div className="relative h-48 overflow-hidden rounded-xl border border-gray-200 shadow-sm bg-white flex items-center">
        <div
        ...
        <div className="flex-1 px-12 py-10 relative z-20">
          <div className="max-w-lg">
            <h2 className="text-[27px] font-semibold text-black mb-4 leading-tight font-sora capitalize ">
              {card.title} <span className=""> {mentor?.full_name || "User"}</span>
            </h2>
            <p className="text-lg text-gray-600 font-raleway mb-8 leading-snug">
              {card.description}
            </p>
        ...
        <div className="flex-1 h-full relative">
          ...
          <Image
            ...
            width={300}
            height={300}
            className=" absolute -top-10 w-full opacity-40 "
          />
        </div>
      </div>
    </div>

...
export default function AILoadingState() {
  ...
  useEffect(() => {
    ...
    }, 1200); // Slightly slower than the example for better readability
    ...
    <div className="flex items-center justify-center min-h-full w-full">
      <div className="space-y-4 w-auto">
        <div className="ml-2 flex items-center space-x-2 text-gray-600 dark:text-gray-300 font-medium">
          ...
          <BsStars className="text-blue-600 mt-1 shrink-0" size={30} />
          <span className="text-lg font-sora capitalize">
        ...
        <div className="relative">
          <div
            ...
            className="font-mono text-sm overflow-hidden w-full h-[84px] relative rounded-lg"
          ...
          >
            <div>
              {visibleLines.map((line, index) => (
                ...
                >
                  <div className="text-gray-600  pr-3 select-none w-6 text-right font-inter">
                    {line.number}
                  </div>

                  <div className="text-gray-800 font-inter text-sm flex-1 ml-1">
                    {line.text}
                  </div>
                </div>
              ))}
          ...
          <div
            className="absolute top-0 left-0 right-0 bottom-0 pointer-events-none rounded-lg from-blue-100/70 via-white/50 to-transparent dark:from-black/90 dark:via-black/50 dark:to-transparent"
            style={{
              background:
